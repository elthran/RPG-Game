...
# TODO use the normal proficiency code for monsters too!
...
class MonsterHealth(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor(modifier*randint(275, 325)*0.01)
...
class MonsterDamage(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.base = floor(3 * (0.5*sin(0.1*modifier) + 0.1*modifier))
        self.final = floor(3 * (0.5*sin(0.1*modifier) + 0.2*modifier)) + 1


class MonsterSpeed(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = round((2.5 * (0.05*sin(0.3*modifier) + 0.08*modifier)) + 1, 2)


class MonsterAccuracy(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor((- (50*5)/((2 * modifier) + 50) + 5))


class MonsterKillshot(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.base = floor((- (5*50)/((0.5 * modifier) + 5) + 50))
...
class MonsterFirstStrike(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor((- (5*50)/((0.3 * modifier) + 5) + 50))


class MonsterDefence(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor((- (7*35)/((0.1 * modifier) + 7) + 35) * 5)


class MonsterEvade(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor((- (10*15)/((0.1 * modifier) + 10) + 15) * 5)


class MonsterParry(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor((- (15*15)/((0.2 * modifier) + 15) + 15) * 5)


class MonsterRiposte(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor((- (20*15)/((0.3 * modifier) + 20) + 15) * 5)


class MonsterFatigue(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.final = floor(modifier) + randint(1, 3)
        self.current = self.final
...
class MonsterBlock(MonsterProficiency):
    def __init__(self, name, modifier):
        super().__init__(name)
        self.base = floor((- (25*60)/((0.25 * modifier) + 25) + 60) * 5)
