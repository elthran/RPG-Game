...
class Abilities(Base):
    ...
    def __init__(self):
        self.scholar = AuraAbility('scholar', 5, 'Gain +{{ level }}% experience per level', learnable=True, understanding_modifier=1)
        self.reflexes = AuraAbility('reflexes', 3, 'Gain +{{ level * 2 }}% dodge chance per level', learnable=True, evade_chance=2)
        self.meditation = AuraAbility('meditation', 10, 'Regenerate 1 sanctity per day', tree='archetype', tree_type='cleric', sanctity_regeneration=1)
        self.explorer = AuraAbility('explorer', 3, 'See 1x more detail when moving to a new place on the map per level', tree='archetype', tree_type='woodsman', map_reveal=1)
        self.cure = CastableAbility('cure', 3, 'Recover 3 health per level', tree='specialization', tree_type='priest', sanctity_cost=1, heal_amount=3)
        self.beastmaster = AuraAbility('beastmaster', 5, 'Take 5% reduced damage from beasts per level', tree='specialization', tree_type='hunter', beast_damage_reduction=5)
