...
class FrozenBlades(AuraAbility):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(name='FrozenBlades', tree='Basic', tree_type='None', max_level=3, description='Gives your weapon a chance of freezing enemies attacked', current='Chance to freeze: {{ level * 5 }}', next='Chance to freeze: {{ (level + 1) * 5 }}', learnable=True, proficiency_data=[('FreezingChance', {'base': 5}), ('Understanding', {'base': 0})])
        for key, value in kwargs:
            setattr(self, key, value)
