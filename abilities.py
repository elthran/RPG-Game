...
class LifestealTest(AuraAbility):
    attrib_name = "lifesteal_test"

    __mapper_args__ = {
        'polymorphic_identity': 'LifestealTest',
    }

    def __init__(self, *args, **kwargs):
        super().__init__(name='LifestealTest', tree='Basic', tree_type='None', max_level=3, description='You steal life per hit', current='Amount stolen: {{ (level) * 1 }}', next='Amount stolen: {{ (level + 1) * 1 }}', learnable=True, proficiency_data=[('Lifesteal', {'base': 1}), ('Understanding', {'base': 0})])
        for key, value in kwargs:
            setattr(self, key, value)
